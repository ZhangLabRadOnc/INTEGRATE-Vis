...
def usage():
    ...
    Optional parameters:
        ...
        -m/--bam-file       [string:    path to BAM file. Plot fusion reads if provided        ]
...
def getParameters(argv):
    try:
        opts, args = getopt.getopt(argv,"hkb:d:g:o:s:r:5:3:m:",["help",
                                                                "keep-tmp",
                                                                "fusion-bedpe=",
                                                                "ideogram=",
                                                                "gene-model=",
...
def plot_one(g5,g3,p5,p3,ideogram,frame_string,gene_model,output_dir,file_name):
    cmd = 'python '+cur+'/pa.py -5 '+g5+' -3 '+g3+' -f '+p5+' -t '+p3+' -p '+frame_string
    if bam_file!='':
        cmd = cmd + ' -b '+bam_file
    cmd = cmd+' -d '+ideogram+' -m '+gene_model+' -o '+file_name
    print "Command:",cmd
...
def plot_all():
    ...
    for x in range(len(all_fusions)):
       ...
       if is_on_boundary(all_fusions[x]):
           ...
           file_name=gen_name(output_dir, sample_name, fusion_name, all_fusions[x], "structure.pdf")
           frame_string=get_frame_str(all_fusions[x])
           if id_5p!='' and id_3p!='':
               if id1==id_5p and id2==id_3p:
                   plot_one(id1,id2,p5,p3,ideogram,frame_string,gene_model,output_dir,file_name)     
           else:
                   plot_one(id1,id2,p5,p3,ideogram,frame_string,gene_model,output_dir,file_name)
...
def main(argv):
    ...
    if len(argv)>1 and argv[1]=="structure":
        global prefix_usage
        prefix_usage="Integrate-vis structure"
        if(len(argv)==2):
            usage()
            return
    ...
    if fusion_bedpe=='' or gene_model=='' or ideogram=='' or reference=='' or sample_name=='':
        usage()
        exit(0);
    print "[Structure] Running Integrate-vis structure on",
    print sample_name,"..."
    use_real_path()
    make_dir(output_dir)
    make_dir(output_dir+'/tmp')
    print "[Structure] Making annotation file..."
    make_annot()
    print "[Structure] Setting gene name dictionaries..."
    set_trans_gene_dict()
    print "[Structure] Plotting figures..."
    print
    plot_all()
    remove_tmp()
    print "[Structure] Total time elapsed:", ("%0.2f" % (time.time()-t)), "seconds."
